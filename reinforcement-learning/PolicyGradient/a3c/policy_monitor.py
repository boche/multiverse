...
class PolicyMonitor(object):
  ...
  """
  def __init__(self, env, policy_net, summary_writer, pps_queue, saver=None):
  ...
  def continuous_eval(self, eval_every, sess, coord):
    ...
    try:
      while not coord.should_stop():
        while not self.pps_queue.empty():
            total_frame_count += self.pps_queue.get()
        duration = time.time() - start_time
        print("%d PPS: %.4f" % (int(duration), total_frame_count / duration)) 
        sys.stdout.flush()

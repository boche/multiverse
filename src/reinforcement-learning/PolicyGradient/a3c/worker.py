...
class Worker(object):
  ...
  """
  def __init__(self, name, env, policy_net, value_net, global_counter, pps_queue, discount_factor=0.99, summary_writer=None, max_global_steps=None):
    self.name = name
    ...
    with sess.as_default(), sess.graph.as_default():
      ...
      try:
        while not coord.should_stop():
          ...
          # print("adding %d transition from thread %s" % (len(transitions), self.name))
          self.pps_queue.put(len(transitions))
